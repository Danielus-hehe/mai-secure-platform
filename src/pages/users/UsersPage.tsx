...
export default function UsersPage() {
    ...
    const handleToggleActive = (u: User) => {
        if (u.isActive) {
            // Dezactivare — necesită confirmare
            setConfirmTarget(u);
        } else {
            // Activare — direct
    ...
    const handleRoleChange = (u: User, newRole: Role) => {
        ...
        refresh();
        toast.info(`Rolul lui ${u.fullName} a fost schimbat în ${ROLE_LABELS[newRole]}.`);
    };
        ...
        <div className="space-y-6">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-mai-50">
                            {users.map((u: User) => (
                                <tr key={u.id} className="hover:bg-mai-50/50 transition-colors">
                                    ...
                                    <td className="px-5 py-3.5">
                                        <select
                                            ...
                                            className={`rounded-full px-2.5 py-1 text-[11px] font-semibold
                                                ...
                                                focus:ring-mai-500 ${ROLE_BADGE_CLASSES[u.role]}`}
                                        >
                                    ...
                                    <td className="px-5 py-3.5 text-right whitespace-nowrap space-x-1">
                                        <Button
                                            ...
                                            onClick={() => toast.info(`Link de resetare parola trimis la ${u.email || u.username}.`)}
                                        >
                                            <KeyRound size={14} />
                                        </Button>
                                        <Button
            ...
            <Modal open={createOpen} title="Creare cont nou" onClose={() => setCreateOpen(false)}>
                <div className="space-y-4">
                    <Input
                        id="fullName" label="Nume complet"
                        value={form.fullName}
                        onChange={e => setForm(f => ({ ...f, fullName: e.target.value }))}
                        placeholder="ex: Ion Popescu" required
                    />
                    <Input
                        id="username" label="Nume de utilizator"
                        value={form.username}
                        onChange={e => setForm(f => ({ ...f, username: e.target.value }))}
                        placeholder="ex: ion.popescu" required
                    />
                    <Input
                        id="email" label="Adresă e-mail"
                        type="email" value={form.email}
                        onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
                        placeholder="ex: ion.popescu@mai.gov.md"
                    />
                    <Input
                        id="department" label="Direcție / Departament"
                    ...
                    <Button
                        ...
                        disabled={!form.fullName || !form.username}
                        className="w-full"
                    >
