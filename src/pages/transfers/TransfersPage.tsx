...
export default function TransfersPage() {
    ...
    // detaliile formularului de upload
    ...
    const handleFilePick = async (f: File | null) => {
        ...
        if (!f) return;
        setComputing(true);
        setHash(await sha256(f));       // SHA-256 calculat în browser
        setComputing(false);
    };
    ...
    const handleSend = () => {
        ...
        const created = transferStore.add({
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
                ...
                ${filter === s ? 'bg-mai-700 text-white' : 'text-mai-600 hover:bg-mai-50'}`}>
                            {s === 'TOATE' ? 'Toate' : s === 'IN_ASTEPTARE' ? 'În așteptare'
                                : s === 'CONFIRMAT' ? 'Confirmate' : 'Expirate'}
                        </button>
                    ))}
                </div>
                        ...
                        return (
                            <tr key={f.id} className="hover:bg-mai-50/50">
                                <td className="px-5 py-3.5">
                                    <p className="font-medium text-mai-900">{f.fileName}</p>
                                    <p className="text-xs text-mai-400">{formatFileSize(f.sizeBytes)}</p>
                                </td>
                                <td className="px-5 py-3.5 text-mai-500">{f.sender.fullName} → {f.recipient.fullName}</td>
                                <td className="px-5 py-3.5 font-mono text-xs text-mai-400">{truncateSha(f.sha256)}</td>
                                <td className="px-5 py-3.5">
                    <span className={`inline-flex items-center gap-1.5 text-xs ${expiringSoon ? 'text-red-600 font-semibold' : 'text-mai-500'}`}>
                                ...
                                <td className="px-5 py-3.5 text-right space-x-1 whitespace-nowrap">
                                    ...
                                    {f.status === 'IN_ASTEPTARE' && (
                                        <Button variant="secondary" className="px-2.5 py-1.5"
                                                onClick={() => { transferStore.confirm(f.id);
            ...
            hover:border-mai-400 transition cursor-pointer p-8 text-center bg-mai-50/50">
                        ...
                        {file ? (
                            ...
                            <p className="text-sm text-mai-500">Trage fișierul aici sau click pentru selectare<br />
                                <span className="text-xs text-mai-300">PDF, DOC(X), XLS(X), ZIP, RAR — max 50 MB</span></p>
                        )}
                    </label>

                    {computing && (
                        <p className="text-sm text-mai-500 flex items-center gap-2">
                            <Clock size={14} /> Se calculează amprenta SHA-256…
                        </p>
                    )}
                    {hash && (
                        <div className="rounded-lg bg-mai-50 border border-mai-100 p-3">
                            <p className="text-xs font-semibold text-mai-700 mb-1">Amprentă SHA-256 (verificare integritate)</p>
