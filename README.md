...
| Integritate documente stocate | Segmente autentificate separat (contor + marcaj „ultimul"); SHA-256 in registru | API-ul refuza livrarea la orice nepotrivire; browserul reverifica |
...
| Semnatura raportata de browser e o afirmatie a clientului | Serverul nu poate verifica singur fara textul in clar | Prin constructie, serverul nu are textul in clar |
...
- **Schimbare parola fortata** (`MustChangePassword`) la creare fara email sau dupa resetare.
...
| `MAI.IntegrationTests` | Teste de integrare cu baza de date in memorie | Api, BusinessLogic |
...
Serverul nu detine, in niciun moment, nimic care sa ii permita sa deschida un fisier transferat: are cheile publice (care cripteaza), blob-uri incuiate cu parole pe care nu le stocheaza si DEK-uri impachetate cu chei private pe care nu le are.
...
| Schimbare parola fortata | `MustChangePassword = true` la creare fara email sau dupa resetare | Validat si pe server, nu doar in UI |
...
**Teste de integrare** (`MAI.IntegrationTests`): fluxuri complete de transfer si resetare parola, cu baza de date in memorie.
...
MAI.IntegrationTests/    teste de integrare (baza in memorie) — transferuri, resetare parola
...
scripts/                 Samba AD de test, recriptare depozit, certificat de demonstratie,
                         seed-demo-objects.sh (fisiere demonstrative in MinIO)
...
```bash
# 1. Fisierele in MinIO (documente descarcabile)
docker compose run --rm minio-init sh /scripts/seed-demo-objects.sh

# 2. Datele in PostgreSQL (utilizatori, transferuri, documente, audit)
psql -h localhost -U sgdm -d sgdm -f MAI.DataAccessLayer/Migrations/900_seed_demo.sql
```

Contul de administrator implicit este creat de `900_seed_demo.sql`. La prima autentificare, interfata cere setarea unei parole proprii inainte de orice alta actiune.
